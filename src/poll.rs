...
impl RegistrationInner {
    ...
    pub fn disable(&self) {
        // Can be called without a poll handle
        self.node().interest.set(EventSet::none());
        self.node().opts.set(PollOpt::empty());
    }
    ...
    fn poll(&mut self, dst: &mut sys::Events) {
        ...
        for node in ready {
            ...
            if event::is_drop(events) {
                ...
                // been reached
                dst.push_event(Event::new(events, node_ref.token.get()));

                // If one-shot, disarm the node
                if opts.is_oneshot() {
                    node_ref.interest.set(EventSet::none());
                }
            }
        }
    }
...
impl ReadinessNode {
    ...
    fn poll_events(&self) -> EventSet {
        (self.interest.get() | event::drop()) & event::from_usize(self.events.load(Ordering::Relaxed))
    }
